...
class Builder{
    ...
    async windows(name)
    {
        ...
        let keyname = newData.keyPath.split('/')[2];
        let check_for_keys = await this.check_if_key_exists(keyname)
        let check_key_existence = check_for_keys[0]
        let private_key_path = check_for_keys[1]
        let public_key_path = check_for_keys[2]
        if (!check_key_existence){
            ...
            console.log(chalk.red('Not regenerating the existing keys. Read below.'));
            console.log(chalk.greenBright(`Please delete the keys present at ${private_key_path}, ${public_key_path} and rerun the pipeline init command to regenerate the keys`));
    ...
    async check_if_key_exists(keyname){
        ...
        // console.log(fs.existsSync(path_private_key))
