...
class Builder{
    async mac(name){
        ...
        child.execSync(`vm run ${name} ubuntu:jammy`,{stdio: 'inherit'});
        // console.log('Changing file permissions');
        // child.execSync(`vm exec ${name} "sudo chmod 777 /etc/resolv.conf"`);
        // console.log('Done with file permissions');
        // child.execSync(`vm exec ${name} "sudo rm /etc/resolv.conf"`);
        // child.execSync(`vm exec ${name} "sudo touch /etc/resolv.conf"`);
        // child.execSync(`vm exec ${name} "echo 'nameserver 8.8.8.8' | sudo tee bad.txt"`);
        let cmd = "echo 'nameserver 8.8.8.8 nameserver 8.8.4.4' | sudo tee /etc/resolv.conf >> /dev/null";
        child.execSync(`vm exec ${name} "${cmd}"`);
