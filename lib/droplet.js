...
class DigitalOceanProvider{
	...
	async keyToDroplet(processor){
		...
		await SSH.executeVM(processor, `sudo chmod 600 ~/.ssh/${vm_name}`)
	...
	async dropletSetup(dropletserver){	
		...
		if(response.status == 202){
			var creationStatusFlag = true;
			
			do{
				let creationCompletionResponse = await axios.get(`https://api.digitalocean.com/v2/droplets/${response.data.droplet.id}/actions`, {headers:headers,})
				.catch(err => console.error(`dropletInfo ${err}`));
				if( !creationCompletionResponse ) return;

				if(creationCompletionResponse.status == 200){
					if( creationCompletionResponse.data.actions ){
						var dropletActions = creationCompletionResponse.data.actions;

						for(var i = 0; i < dropletActions.length; i++){
							if (dropletActions[i].type == "create"){
								if (dropletActions[i].status == "completed"){
									creationStatusFlag = false;
									console.log(chalk.gray('created'));
								} else{
									await new Promise(resolve => setTimeout(resolve, 10000));
									console.log(chalk.gray(dropletActions[i].status));
								}
							break;
							}
						}
				...
				if( getDropResponse.data.droplet ){
					...
					droplet.networks.v4.forEach(i => {
						if(i.type == 'public'){
							...
							console.log(chalk.green(`Droplet Data for ${dropletserver}:`, JSON.stringify(inventoryDic)));
							inventory[dropletserver]={"DropletID": response.data.droplet.id, "ip": i.ip_address}
						}
					})
				}
			}
