...
class Deployment{
    async deployJob(processor, jobName,buildFile){
        // let dropletsList=["droplet-blue", "droplet-green", "monitor"]
         ...
         for(let k in inventoryJSON){
            // console.log(k, inventoryJSON[k].ip)
            if(k=='monitor'){
                // console.log(k, inventoryJSON[k].ip)
    ...
    async monitorSetup(processor, ip){
        ...
        await SSH.executeDroplet(processor, `sudo apt-get -qqq install nodejs -y`, ip);
        await SSH.executeDroplet(processor, `sudo apt-get -qqq install npm -y `, ip);
    }
    ...
    async deployDroplet(processor, jobName, buildFile, ip){
        ...
        for(let i=0; i< jobs.length;i++){
            if(jobs[i].name == jobName){
                ...
                await SSH.executeDroplet(processor, `dos2unix ${server}.sh`);
