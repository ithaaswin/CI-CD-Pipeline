    ...
    async deployDroplet(processor, jobName, buildFile, ip){
        ...
        for(let i=0; i< jobs.length;i++){
            if(jobs[i].name == jobName){
                ...
                var app = jobName.split("-");
                var permission = fs.readFileSync(`${app[0]}Permission.json`);
                permission = JSON.parse(permission)[app[0]];
                if(permission == "Not Permitted"){
                    console.log(chalk.red('Code Coverage below Threshold. Deployment not permitted'));
                    process.exit();
                }
