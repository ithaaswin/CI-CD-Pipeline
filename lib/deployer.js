    ...
    async deployJob(processor, jobName, buildFile){
        ...
        for(let i=0; i< jobs.length;i++){
            if(jobs[i].name == jobName && jobName == "itrust-deploy"){
                ...
                await SSH.executeVM(processor, `scp -i ~/.ssh/${process.env.vm_name} -r ${sharedPath}/scripts/tomcat.sh root@${ip}:`, ip);
                await SSH.executeDroplet(processor, `sudo bash tomcat.sh`, ip);
                await SSH.executeVM(processor, `scp -i ~/.ssh/${process.env.vm_name} -r ~/iTrust2-v10/iTrust2/target/iTrust2-10.war root@${ip}:/opt/tomcat/webapps`);
