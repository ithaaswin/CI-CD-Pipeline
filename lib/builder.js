...
class Builder{
    async mac(job_name, build_path){
        ...
        child.execSync(`ansible-playbook lib/${build_path} --tags "${job_name}" -i inventory --extra-vars "git_user=${process.env.git_user} git_token=${process.env.git_token} db_user=${process.env.db_user} db_pass=${process.env.db_pass}"`,{stdio: ['inherit', 'inherit', 'inherit']});
    }
