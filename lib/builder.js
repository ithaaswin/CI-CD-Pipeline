...
class Builder{
    ...
    async windows(job_name, build_path)
    {
        ...
        let config_port = JSON.parse(configJSON).config_port;
        // child.execSync(`bakerx run  ${vm_name} focal`, {stdio: 'inherit'});
        child.execSync(`ssh -q -i "~/.bakerx/insecure_private_key" -p 2003 -o StrictHostKeyChecking=no vagrant@127.0.0.1 "ansible-playbook /bakerx/lib/build.yml --tags iTrust -i /bakerx/inventory --extra-vars 'git_user=hhasti@ncsu.edu git_pass=Kavitha@8'"`, {stdio: 'inherit'});
