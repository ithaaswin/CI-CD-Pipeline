...
class Builder{
    ...
    async windows(job_name, build_path)
    {
        ...
        child.execSync(`ssh -q -i "~/.bakerx/insecure_private_key" -p ${config_port} -o StrictHostKeyChecking=no vagrant@127.0.0.1 "ansible-playbook /bakerx/lib/build.yml --tags "${job_name}" -i /bakerx/inventory"`, {stdio: 'inherit'});
