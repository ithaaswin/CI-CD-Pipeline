...
class Builder{
    ...
    async build_job(processor, job_name, build_path) {
        ...
        await vmSSH.executeVM(processor, `sudo cp -rp ${sharedPath}/scripts/dpkgUnlocker.sh .`)
        await vmSSH.executeVM(processor, `sudo bash dpkgUnlocker.sh "${user}"`);
        ...
        for(let i=0; i< jobs.length;i++){
            ...
            if(jobs[i].name == job_name && job_name == "itrust-build"){
                ...
                for(let j=0;j<steps.length;j++){
                    ...
                    await vmSSH.executeVM(processor, cmd)
                }
                ...
                await vmSSH.executeVM(processor, `sudo cp -rp ${sharedPath}/mutation/. .`);
                await vmSSH.executeVM(processor, `sudo bash ~/mutate.sh "${url}" "${sharedPath}" "${iterations}" "${user}"`);
            }
        }
    }
