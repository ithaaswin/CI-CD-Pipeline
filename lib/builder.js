...
class Builder{
    ...
    async build_job(processor, build_path, job_name) {
        ...
        await vmSSH.execute(processor, true, `sudo cp -rp ${sharedPath}/scripts/dpkgUnlocker.sh .`)
        await vmSSH.execute(processor, true, `sudo bash dpkgUnlocker.sh "${user}"`);
        
        for(let i=0; i < setup.length; i++) {
            setup[i] = setup[i].replace("{{mysql_pass}}", mysql_pass);
            await vmSSH.execute(processor,false, setup[i])
        }

        for(let i=0; i< jobs.length;i++){

            if(jobs[i].name == job_name && job_name == "itrust-build"){
                ...
                for(let j=0;j<steps.length;j++){
                    ...
                    await vmSSH.execute(processor, false, cmd)
                }
            }
            else if(jobs[i].name == job_name && job_name == "mutation-coverage"){
                ...
                await vmSSH.execute(processor, true, `sudo cp -rp ${sharedPath}/mutation/. .`);
                await vmSSH.execute(processor, true, `sudo bash ~/mutate.sh "${url}" "${sharedPath}" "${iterations}" "${user}"`);
            }
        }
    }
