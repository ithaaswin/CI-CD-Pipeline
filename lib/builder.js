...
class Builder{
    constructor() {
        ...
        let ip_address=jsonData.HostName;
        let user=jsonData.user;
        let file_path=jsonData.IdentityFile;
        child.execSync(`echo [Web] > inventory`);
        child.execSync(`echo ${ip_address} ansible_ssh_user=${user}  ansible_ssh_private_key_file=${file_path} >> inventory`);
