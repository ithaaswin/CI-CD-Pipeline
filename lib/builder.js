...
class Builder{
    ...
    async writeToJson(jsonData){
        console.log(jsonData);
        fs.writeFileSync("snapshot.JSON", jsonData); 
        console.log("data saved to snapshot.JSON");
            ...
            for(let i=0; i< jobs.length;i++){
                if (jobs[i].name == job_name){                  
                    if(jobs[i].name == "mutation-coverage") {
                        ...
                        {
                            let key = k
                            let value = snapshots[k]
                            dic[key] = value
                        }
                        console.log(chalk.red(typeof snapshots))
