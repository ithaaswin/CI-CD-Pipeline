    ...
    async build_job(processor, job_name, build_path) {
        ...
        for(let i=0; i< jobs.length;i++){
            ...
            if(jobs[i].name == job_name && job_name == "itrust-build"){
                ...
                await SSH.executeVM(processor, `sudo dos2unix mutate.sh`)
                await SSH.executeVM(processor, `sudo dos2unix randomSelector.js`)
                await SSH.executeVM(processor, `sudo dos2unix files_ignore.txt`)
